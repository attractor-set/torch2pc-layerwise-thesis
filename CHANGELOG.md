...
Новых изменений после `v1.0.1` пока нет.

## [1.0.1] — 2026-08-18

### Полная английская версия диссертации

- Добавлено полное английское представление диссертации с отдельной точкой входа `thesis/main_EN.tex` и англоязычными главами, приложениями и генерируемыми таблицами.
- Добавлена машинная проверка RU/EN структурной и научно-семантической конгруэнтности; статусы C01–C11 и границы выводов не изменены.
- Релизный контракт теперь публикует два явно маркированных PDF-артефакта, `-ru.pdf` и `-en.pdf`, с независимыми SHA-256 и общим commit/tree manifest.
- Научное выполнение не переоткрывается; `v1.0.1` является языковым/публикационным расширением закрытого научного состояния `v1.0.0`.

## [1.0.0] — 2026-08-17

### Финальный релиз диссертации

- Зафиксирована интегрированная 99-страничная версия диссертации после независимого научного T24-аудита.
- Семантика QWake-FP согласована с зарегистрированным bounded analytic completion и exact-suffix reference/fallback.
- Сохранены зарегистрированные статусы C01–C11, отрицательный C09 и границы `not_tested` для C10/C11.
- Добавлена машинная проверка релизного контракта и публикация source/PDF assets через тег `v1.0.0`.

### Добавлено

- Материализована двухфайловая фиксация состояния репозитория `QW-LC2`, связывающая проверенный resource-cost контракт с `8f24229bcf19736086fe6f0340bda26dd533936a`; `QW-LC3`, реализация и выполнение закрыты.
- зафиксирован контракт ресурсов и стоимости `QW-LC2`: канонический `Γ(a,s;r,p)`, недублирующее отображение `Φ`, 11-полевой `C`, инженерный shadow и будущий end-to-end профили, zero-safe `~C`, Pareto-правило и deterministic tie-break; `QW-LC3`, реализация и выполнение остаются закрытыми;

- материализован переход `QW-LC2`: завершённый `QW-LC1` связан с двухфайловой квитанцией, будущий контракт ограничен схемой `Γ(a,s)`, отображением `Φ` и оператором `~C`, а поля, единицы, стоимость, реализация и выполнение остаются закрытыми до слияния перехода;

- материализована фиксация состояния репозитория `QW-LC1`: схема из коммита `de2b5a37583b22946073390caa244bee35dd793b` повторно проверена на `main` `59e3143ba105a5b298e2cd551b221b8f6dae96f7`, создана отдельная двухфайловая квитанция, а завершение `QW-LC1`, переход к `QW-LC2`, ресурсная траектория, стоимость, реализация и выполнение остаются закрытыми до слияния и повторной проверки;
- зафиксирована схема требуемого результата `QW-LC1`: контракт `stage3b-qwake-lc1-required-response-schema-v1` определяет упорядоченный `R(a,s)` из именованных градиентов, конечных beliefs и scalar loss, каноническую manifest/payload сериализацию, обязательные observables, zero-safe оператор `~R` и профили CPU/ROCm; `Γ`, стоимость, реализация, выполнение и научные утверждения остаются закрытыми;
- материализован переход `QW-LC1`: завершённая фиксация состояния репозитория `QW-LC0` связана с двухфайловой квитанцией, область будущего среза ограничена схемой `R(a,s)`, обязательными наблюдаемыми полями и оператором `~R`, а сам `QW-LC1`, код и выполнение остаются закрытыми до слияния перехода;
- материализована фиксация состояния репозитория `QW-LC0`: контракт из коммита `715308451ac3e696d4c2209276d36853f6799d6f` подтверждён на `main` `8429f54257685a879b0a44499d5fa81eab7310ea`, создана отдельная двухфайловая квитанция, а переход к `QW-LC1`, реализация, выполнение и научная кампания остаются закрытыми до слияния и повторной проверки;
- материализована нормативная фиксация `QW-LC0`: запечатан контракт `stage3b-qwake-lc0-semantics-scope-v1`, разделяющий `R/M/Γ/C`, ограничивающий первый аналитический кандидат и сохраняющий реализацию, выполнение и научную кампанию закрытыми;
- после слияния PR #110 зафиксирован post-merge переход `QW-4B-E-v2`: repository evidence повторно проверены на `main` `4f23b752a40ae05de9fc7ee49c9962c44083b71d`, `QW-LC0` открыт только для фиксации семантики и области, а реализация, выполнение, научный образ, test split и публикация остаются закрытыми;
- выполнен и материализован `QW-4B-E-v2`: единственная разрешённая попытка завершила шесть ячеек CPU/ROCm × P0/P1/P2, точный output связан с report `sha256:54dba01d47814dc00fa53bd69c00865bd1c47754c017c7482c895162d3a86b82`; исходная ошибка wrapper, два неудачных recovery-аудита и успешный recovery-v3 сохранены, authorization использовано, повтор запрещён, scientific evidence и публикация закрыты;
