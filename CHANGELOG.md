...
- реализован backend-neutral `QW-3` superset pipeline без Torch2PC/GPU effects: закрытый component registry, effect-local planning, exact `A0/A1/A2` trajectory schema, finite policy interpreter, deterministic B0–B7 и nested-ablation replay, cost mapping, opportunity/recognizability, shadow/replication evaluation, pure sealing и rendered-not-published export; live adapters и scientific execution остаются закрытыми;
- заморожен машиночитаемый контракт `QW-2` для единственного обязательного `QWake-FP` special case: `FixedPred`, `eta=1`, `stage2_baseline`, `lenet_classic`, EX-IF0 endpoint defect, точные накопительные `A0/A1/A2`, конечный analytic registry, B0–B7, P0–P2, недублирующее cost mapping и унаследованные из QW-1 role/receipt guards; scientific execution остаётся закрытым;
