...
`src/torch2pc_thesis/__init__.py` входит в зафиксированный научный контур
выполнения QWake и поэтому сохраняет историческое `__version__ = "0.1.0"`.
Версия репозитория/релиза определяется `pyproject.toml`, `CITATION.cff` и тегом;
менять этот зафиксированный исходный файл только ради синхронизации номера
версии нельзя.
