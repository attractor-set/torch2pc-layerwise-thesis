...
глубине/ширине. Подробный протокол: [docs/stage-3-protocol.md](docs/stage-3-protocol.md).

Текущее состояние разрешает начало реализации и profiling, но намеренно
блокирует pilot/final execution:
...
Детерминированный design plan содержит 288 коротких profiling cells и 48
parameterized validation-only screening cells. Stage 3 отсутствует из `TRAINING_STAGES`, а final
template сохраняет `evaluation.use_test=false` до отдельного freeze.
