...
нейтральные к политике ветви `stop`, `native_one` и `exact_one`, сохраняя
полезность, regret решения, временную историю, стоимость признаков, переходы
и происхождение. Независимая единица — `model_seed`; тестовая выборка закрыта.

## Этап 18 — `A11-OFF1`, предиктор (`predictor`), точная проверка и теневой режим (`shadow`) `QWake-PC`

- провести офлайн-отсев вложенных представлений, признаков и порогов по regret,
  опасным пропускам и полному вектору стоимости;
