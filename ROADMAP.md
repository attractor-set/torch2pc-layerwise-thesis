...
Этот раздел определяет **текущую** последовательность после научного закрытия
T24. Существующие Stage/QWake этапы ниже сохранены как исторический roadmap и
не должны трактоваться как ещё не выполненный план `v1.0.1`.

### R0 — публикация двуязычного `v1.0.1`

`v1.0.0` уже опубликован как научная точка закрытия. `v1.0.1` завершает
публичную языковую поверхность без изменения научного содержания:

1. добавить полное английское представление всех глав и приложений;
2. связать RU/EN структуру, C01–C11, терминологию и критические численные границы машинным congruence gate;
3. собирать русский и английский PDF из одного exact release commit/tree;
4. публиковать отдельные `-ru.pdf` и `-en.pdf` с собственными SHA-256 в одном release manifest;
5. сохранить frozen scientific runtime identities и исторический `v1.0.0` неизменными.

Этот этап не изменяет C01–C11 и не открывает scientific execution.

### R1 — новый эксперимент для C10

Следующий научный вопрос должен быть новым preregistered experiment, а не
пересчётом C2: измерить **добавочную стоимость фактического минимального
распознавателя** и сравнить её со стоимостью предотвращённого residual compute.
Новый протокол обязан заранее зафиксировать estimand, аппаратную среду,
calibration/confirmatory split, dangerous-accept criterion и stop rules.

### R2 — независимая подтверждающая поверхность

Только если новый recognizer удовлетворяет заранее заданному safety/cost
критерию, разрешается отдельная confirmatory campaign на новой поверхности.
Она не называется ретроспективно `C3` исходной C2-цепочки и получает новый
protocol ID.

### R3 — прямая эмпирическая проверка PC-CATM

PC-CATM остаётся теоретико-диагностическим уровнем. Для усиления теории нужен
отдельный preregistered comparison, в котором `NCZ`/`ECZ`/`TNZ`, `alpha_l`,
`m_l`, `chi_l` или связанные channel/transport/cancellation признаки делают
различимое предсказание и сравниваются с более простыми временными признаками.

### R4 — внешняя валидность

Репликации на других архитектурах, datasets, численной точности, аппаратных
средах и реализациях PC должны расширять область применимости, не переписывая
существующие bounded claims.

### Исторический roadmap

Ниже сохраняется прежняя поэтапная карта. Она важна как provenance
последовательности допусков, поэтому её historical tokens и закрытые состояния
не удаляются даже после завершения соответствующих этапов.
