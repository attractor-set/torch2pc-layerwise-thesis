...
Условие открытия выполнено. Текущий slice замораживает 288-cell matrix для
`stage2_baseline`, `isolated_layer_vjp` и `composite_vjp`, а также request,
который различает scientific admission и runtime authorization.

После этого разрешены candidate-aware runner и отдельная ROCm/float32 runtime
freeze. До неё measurements не выполняются. A0, `EX-IF0`, estimator, active
`ECZ`, `QWake-PC`, controller actions и test split остаются закрытыми.
