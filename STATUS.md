...
| Предварительная регистрация B1/B2 | опубликована под `stage3b-b1-b2-prereg-v1` |
...
B1 и B2 прошли отдельные CPU `float64` и ROCm `float32` smoke lanes. Sealed
решения `EQ-B1` и `EQ-B2` имеют `status=pass`, `failed_pairs=[]`; все
зарегистрированные gates прошли. Это открывает только matched profiling и не
является доказательством runtime/memory benefit.
