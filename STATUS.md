...
## Текущий статус v1.0.1 — 18 августа 2026 года

Этот раздел является **авторитетным текущим статусом** репозитория. Научное
содержание диссертации остаётся точкой закрытия T24 и не изменяется в
`v1.0.1`. Выпуск `v1.0.1` добавляет полное английское представление той же
диссертации, RU/EN machine-checkable congruence gate и отдельные `-ru.pdf` /
`-en.pdf` release assets. Ни одно утверждение C01–C11, его epistemic status,
экспериментальный результат или scientific execution не изменяются и не
переоткрываются.

Авторитетные исходные точки документов: `thesis/main.tex` (RU) и
`thesis/main_EN.tex` (EN). Оба представления строятся из одной замороженной
машиночитаемой научной поверхности; равенство нумерации страниц не является
критерием семантической эквивалентности.

## Научная точка закрытия v1.0.0 — 17 августа 2026 года

Этот исторический блок фиксирует научную точку закрытия, опубликованную в
`v1.0.0`. Он сохраняется для трассировки и не переопределяет текущую
двуязычную публикационную поверхность `v1.0.1`.
...
Они являются provenance, а не текущим состоянием `v1.0.1`.
...
## `QW-5`: единый научный образ зафиксирован (ADR-123)

`QW-5` завершён через отдельно доказанную корректирующую фиксацию неизменяемого
образа `sha256:800471114a6fec7d401fcbc3c781957265aaa351babaa5adf823d1521a95a8e3`. Исходный `Attempt-001` остаётся терминальной
неуспешной попыткой: `success=false`, `reinterpreted=false`,
`retry_performed=false`. Один и тот же идентификатор образа обязателен для
`C1/C2/C3/R`; `C1` ещё не открыт.
...
## `QW-5`: подготовлен замещающий научный оркестратор (ADR-124)

Структурная проверка готовности `C1` показала, что образ ADR-123 содержит
закрытую модель возможностей и научные компоненты, но не содержит заранее
зарегистрированной точки запуска научной кампании. Образ версии 1 и его
корректирующая фиксация сохраняются неизменными.

ADR-124 добавляет только исходный код общего хостового и встроенного
оркестратора, закрытые канонические запросы и разрешения, одноразовую хостовую
фиксацию и точную цепочку квитанций. Новый образ в этом срезе не строится;
`C1/C2/C3/R`, научное выполнение, доступ к тестовой выборке и публикация
остаются закрытыми.
...
## `C1 Attempt-001`: terminal permission failure и host identity correction (ADR-129)

Attempt-001 терминально потребил authorization `sha256:1d62b313f609be9b9ba9a813761c672e1866c35ca794253babb6c829f42fdc6b` и не может быть повторён. ADR-129 исправляет host/container DAC identity contract без ослабления `cap-drop=ALL`.
...
## `C1`: новый request-freeze интегрируется в репозиторий (ADR-131)

Новый `C1_COLLECTION` request уже зафиксирован и независимо проверен после
host-identity correction и новой scientific-image freeze. Его semantic identity
— `sha256:af7c27ec0db83d907b51361a8bb726db51f41fdbdc6bc341156b420648c606fd`, request freeze — `sha256:7fe57faefcc3aa92463e01422546e015e02e7a68c13dbbc9df1ef9feb7452b82`.
Интеграция сохраняет исходные evidence bytes и не открывает Attempt-002.
