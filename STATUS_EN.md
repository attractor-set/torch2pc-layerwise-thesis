...
- formalize B1 and B2 relative to the published B0 baseline;
- implement each candidate separately;
- apply preregistered cosine, relative-L2, finite-value, and stability criteria;
- keep test-dataset access disabled;
- run a small profiling pilot only for candidates that pass the numerical
  checks;
...
dependency radius, graph span and lifetime, the feedback operator, and
