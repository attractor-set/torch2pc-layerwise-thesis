...
raw records as non-evidence execution artifacts. Those records deliberately
...
1. requires the expected execution source commit, sealing implementation
...
The raw archive remains immutable and retains its original non-evidence flags.
...
The final field makes clear that only the B0 baseline candidate is complete,
not the broader Stage 3B candidate campaign.

## Statistical unit

`model_seed` remains the statistical unit. Repetitions, measured steps, and
profiling regions are repeated observations within a seed. FixedPred/Strict
paired tables are joined by depth, width, batch size, and model seed.

## Consequences

- Execution code does not change publication status.
- Raw records and Stage 3A evidence remain unchanged.
- Compact derived tables can be versioned and published separately from the
  large raw archive.
- Any archive mutation, missing attempt, failed terminal, process-isolation
  violation, or numerical-integrity failure blocks sealing.
- The seal records separate execution and sealing implementation commits.
