...
- **Статус:** принят как переходный контракт; инженерный запуск не выполнен
- **Дата:** 2026-08-02
- **Контекст:** `QW-LC4-E`
...
файл владения v2, устойчивая квитанция исхода хоста и runtime output отсутствуют.
...
Запечатанный evidence-пакет подтверждает:
...
1. Считать линию materialization callsite завершённой, потреблённой и
   невозобновляемой.
2. Не классифицировать её evidence-пакет как инженерный отчёт расширения.
3. Оставить `QW-5`, `C1`, `C2`, `C3`, `R`, test split и публикацию закрытыми.
4. Запретить повторный вызов уже выполненной production callsite, повторное
   потребление её разрешения и автоматический либо ручной retry на основании
   той же записи.
5. Разрешить только отдельный будущий authoring-срез допуска фактического
   одноразового инженерного запуска через lease-bound host invoker.
6. Обязать будущий срез связать материализованное подтверждение, persistent
   evidence chain v2, точный образ, Torch2PC, каталог результата и новое
   отдельное одноразовое разрешение.
7. Не создавать в этом переходном срезе lease, host outcome, runtime output,
   команду Docker или научные результаты.
8. Не изменять существующие frozen-пакеты и evidence.
...
Этот будущий authoring-срез не выполняет модель. Он только фиксирует
...
авторизации, терминального evidence и repository seal.
