...
publication_permitted=false
```
## 13. Нормативная схема `QW-LC1`

`stage3b-qwake-lc1-required-response-schema-v1` превращает краткое описание `QW-LC1` в точный контракт.
Компоненты ответа сравниваются по отдельным зарегистрированным записям, поэтому
ошибка малого параметра или слоя не может быть скрыта глобальной нормой.
Структурное несовпадение, отсутствующая запись, неконечное численное значение или
случай одной нулевой нормы немедленно дают отказ.

Пороговые профили:

| Профиль | `max_abs` | `max_relative_l2` | `min_cosine` | `zero_atol` |
|---|---:|---:|---:|---:|
| `cpu_float64_engineering` | `1e-9` | `1e-7` | `0.99999` | `1e-12` |
| `rocm_float32_canonical` | `1e-5` | `1e-3` | `0.999` | `1e-7` |

ROCm/float32 остаётся решенческим профилем. CPU/float64 — инженерный контроль.
Ни один профиль не подтверждает фактическую эквивалентность до будущей
сопоставленной проверки.
