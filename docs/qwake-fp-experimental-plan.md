...
`QW-4B` отдельным решением о допуске связывает адаптеры и выполняет инженерную
проверку `CPU` и каноническую проверку `ROCm/float32`. До появления запечатанного
отчёта выполнение, заморозка образа и кампании `C1/C2/C3/R` остаются закрытыми.
