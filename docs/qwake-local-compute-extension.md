...
отделяет завершённую production callsite materialization-попытку от ещё
отсутствующего инженерного отчёта расширения. Потреблённая авторизация этой
точки вызова не может использоваться повторно. `QW-5` остаётся закрытым до
отдельного одноразового инженерного запуска через lease-bound host invoker,
терминального отчёта и repository seal.
