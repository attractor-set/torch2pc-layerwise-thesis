...
a separate compact evidence bundle.

Required identity inputs are:

- the exact execution source commit;
...
- `configuration_metrics.csv` — 32 three-seed method/configuration summaries;
- `seal.json` — content-addressed evidence seal with execution and sealing source commits;
...
The seal permits publication only for aggregate B0 baseline-candidate
evidence. It does not mark A0/B1/B2 or the full Stage 3B campaign complete. The
raw archive remains a non-evidence execution source and is not committed to
Git.
