...
for lane, expected_lane in zip(lanes, expected_lanes):
    ...
    for item in cells:
        ...
        if not isinstance(audits, list):
            raise RuntimeError(
                "disabled-capability audit registry is not a list"
            )
        ...
        expected_audit_capabilities = (
            reference_disabled
            + instrumented_disabled[pair_id]
        )
        observed_audit_capabilities = tuple(
            audit.get("capability")
            for audit in audits
        )
        if observed_audit_capabilities != (
            expected_audit_capabilities
        ):
            raise RuntimeError(
                "disabled-capability audit registry differs"
            )
        ...
        for audit in audits:
            if not isinstance(audit, dict):
                raise RuntimeError(
                    "disabled-capability audit is not an object"
                )
            if set(audit) != {
                "capability",
                "enabled",
                ...
                raise RuntimeError(
                    "disabled-capability audit fields differ"
                )
            effects = audit.get("effects")
            if not isinstance(effects, dict):
                raise RuntimeError(
                    "disabled-capability effects are invalid"
                )
            if set(effects) != expected_effect_fields:
                raise RuntimeError(
                    "disabled-capability effect fields differ"
                )
            if any(
                ...
                raise RuntimeError(
                    "disabled-capability effect type differs"
                )

            reconstructed = DisabledCapabilityAudit(
                capability=Capability(audit["capability"]),
                enabled=audit["enabled"],
                effects=EffectAudit(**effects),
            )
            validate_disabled_capability(reconstructed)
            if not reconstructed.passed:
                raise RuntimeError(
                    "disabled capability audit did not pass"
                )
...
print("OK: independent recovery-v3 audit passed")
