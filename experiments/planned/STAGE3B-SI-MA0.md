...
## Поправка к версии v1

Эта версия заменяет `stage3b-si-ma0-v1`, запечатанную тегом
`stage3b-si-ma0-prereg-v1`. Поправка сделана до первого implementation smoke
и до появления любых результатов `SI-MA0`.

В frozen `StrictPCPredErrs` модель из шести модулей обновляет пять внутренних
belief states на каждом sweep. Ошибка выходного слоя вычисляется один раз на
sweep, но не передаётся в belief update map. Поэтому версия v1 ошибочно
считала шесть reconstruction/update events на sweep.
...
Общее число diagnostic records остаётся равным 3600, но `REC-MA0` применяется
только к 3000 фактическим state-update events. Выходные ошибки фиксируются в
600 отдельных records и проверяются на полноту, порядок, finite values и
provenance без фиктивного `u_observed` или фиктивного state transition.
...
## Канонические записи sweep

### State-update event

Для каждого из пяти фактических belief updates на
`(model_seed, batch_id, sweep_index, layer_index)` сохраняются:
...
Для каждого sweep отдельно сохраняется ошибка выходного слоя:
...
Output-error record не является state-update event. Для него не создаются
`u_observed`, canonical channel pair или state-transition metrics.

## Реконструкция поправки

Для каждого state-update event реализация обязана экспортировать:
...
Проходит, если на всех 3000 confirmatory state-update events:
...
Отдельно ожидаются 600 output-error records:

```text
10 model seeds * 3 batches * 20 sweeps = 600
```

Они входят в `VER-MA0` и `CMP-MA0`, но не в reconstruction conjunction.
...
- version fields присутствуют на каждом state-update event;
...
- на каждый sweep присутствует ровно один output-error record до пяти
  state-update events;
- output-error records имеют finite values и совпадающий snapshot fingerprint.
...
- все ожидаемые state-update, output-error и timing records присутствуют;
