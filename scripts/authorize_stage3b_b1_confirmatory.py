#!/usr/bin/env python3
"""Freeze, preflight, authorize, and verify confirmatory EQ-B1 execution."""
...
from torch2pc_thesis.stage3b_b1_confirmatory_authorization import (
    B1_CONFIRMATORY_DEFAULT_MINIMUM_FREE_BYTES,
    B1_CONFIRMATORY_EXECUTION_MODES,
    capture_b1_confirmatory_lane_preflight,
    freeze_b1_confirmatory_project,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-root",
        type=Path,
        default=Path("/tmp/torch2pc-stage3b-b1-confirmatory-v1"),
    )
    ...
    parser.add_argument(
        "--minimum-free-bytes",
        type=int,
        default=B1_CONFIRMATORY_DEFAULT_MINIMUM_FREE_BYTES,
    )
    parser.add_argument("--operator-acknowledgement")
    parser.add_argument(
        "--execution-mode",
        choices=B1_CONFIRMATORY_EXECUTION_MODES,
...
def main() -> int:
    ...
    if args.freeze_project:
        payload = freeze_b1_confirmatory_project(
            request,
    ...
    if args.preflight_lane:
        ...
        payload = capture_b1_confirmatory_lane_preflight(
            freeze,
            request,
            project_root=args.project_root,
            torch2pc_dir=args.torch2pc_dir,
    ...
    if args.issue_authorization:
        ...
        payload = issue_b1_confirmatory_authorization(
            freeze,
            preflights,
    ...
    payload = verify_b1_confirmatory_authorization_for_lane(
        authorization,
        request,
        project_root=args.project_root,
        torch2pc_dir=args.torch2pc_dir,
