...
def render_program(core: dict[str, object], qwake: dict[str, object]) -> str:
    ...
    lines = [
        ...
        r"\begin{tabular}{p{0.14\textwidth}p{0.23\textwidth}p{0.18\textwidth}p{0.34\textwidth}}",
        r"\toprule",
        ...
        texrow(f"QWake C2 & {int(selection['candidate_count'])} frozen policies; 756 records & safe recognition/economics & bounded negative result; C3 closed"),
        r"\bottomrule",
        r"\end{tabular}",
        r"\end{table}",
        "",
...
def render_stage3(core: dict[str, object]) -> str:
    ...
    lines = [
        ...
        r"\begin{tabular}{p{0.15\textwidth}p{0.72\textwidth}}",
        r"\toprule",
