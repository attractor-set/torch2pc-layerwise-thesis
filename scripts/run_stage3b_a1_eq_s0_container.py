...
def verify_controlled_image(repo: Path) -> tuple[str, str]:
    ...
    if dirty:
        raise RuntimeError(
            "Canonical EQ-S0 execution requires a clean committed tree. "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run Stage 3B EQ-S0 through the controlled Docker execution lane."
    )
...
def main() -> None:
    ...
    command = [
        ...
        service,
        "python",
        "scripts/run_stage3b_a1_eq_s0.py",
        args.device,
        "--max-batches",
    ...
    summary_path = repo / output_dir / "eq_s0_summary.json"
    ...
    if observed != expected:
        raise RuntimeError(
            f"EQ-S0 output provenance mismatch: expected {expected}, observed {observed}"
        )
    print(f"OK: controlled EQ-S0 {args.device} gate passed with verified provenance")
