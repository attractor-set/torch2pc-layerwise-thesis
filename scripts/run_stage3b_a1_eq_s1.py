...
from torch2pc_thesis.stage3b_a1_shortcut import evaluate_eq_s1
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run Stage 3B EQ-S1: BP versus the opt-in reduced shortcut "
            "with one joint VJP per top-level layer."
        )
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            "results/stage-3/a1-shortcut-observer-controls/working/eq-s1"
        ),
    )
...
def main() -> None:
    ...
    for model_seed in model_seeds:
        ...
        if processed != args.max_batches:
            raise RuntimeError("The training loader produced too few EQ-S1 batches")
    ...
    summary = {
        "control_id": "EQ-S1",
        "scope": (
            "BP vs opt-in reduced shortcut using one joint VJP per "
            "top-level layer"
        ),
        "candidate_contract": {
        ...
        "claim_boundary": (
            "This gate concerns BP/reduced-shortcut endpoint gradients and one "
            "stateless SGD step in the pinned environment. It does not establish "
            "hidden-state trajectory equivalence, training-trajectory equivalence, "
            "runtime benefit, or EQ-S2."
        ),
    }

    records_frame.to_csv(output_dir / "eq_s1_records.csv", index=False)
    (output_dir / "eq_s1_summary.json").write_text(
