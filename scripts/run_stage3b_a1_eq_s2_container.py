...
def verify_controlled_image(repo: Path) -> tuple[str, str]:
    ...
    if dirty:
        raise RuntimeError(
            "Canonical EQ-S2 execution requires a clean committed tree. "
...
def main() -> None:
    ...
    command = [
        ...
        service,
        "python",
        "scripts/run_stage3b_a1_eq_s2.py",
        args.device,
        "--max-batches",
    ...
    if observed != expected:
        raise RuntimeError(
            f"EQ-S2 output provenance mismatch: expected {expected}, "
            f"observed {observed}"
        )
    if summary.get("control_id") != "EQ-S2" or summary.get("passed") is not True:
        raise RuntimeError("EQ-S2 output does not contain a passing EQ-S2 summary")

    runs = summary.get("runs")
    model_seeds = summary.get("model_seeds")
    batches_per_seed = summary.get("batches_per_seed")
    if not isinstance(runs, list) or not runs:
        raise RuntimeError("EQ-S2 output contains no run summaries")
    if not isinstance(model_seeds, list) or not isinstance(batches_per_seed, int):
        raise RuntimeError("EQ-S2 output has invalid run-count metadata")
    expected_runs = len(model_seeds) * batches_per_seed
    if len(runs) != expected_runs:
        raise RuntimeError(
            f"EQ-S2 output contains {len(runs)} runs; expected {expected_runs}"
        )

    for run in runs:
        if run.get("eta") != 1.0:
            raise RuntimeError("EQ-S2 output has an invalid FixedPred eta")
        ...
        inference_steps = run.get("inference_steps")
        if diagnostics.get("one_call_per_layer") is not True:
            raise RuntimeError("EQ-S2 output violated the one-joint-VJP-per-layer contract")
        if not isinstance(top_level_layers, int) or top_level_layers < 1:
            raise RuntimeError("EQ-S2 output has invalid top-level-layer diagnostics")
        if joint_vjp_calls != top_level_layers:
            raise RuntimeError("EQ-S2 output has inconsistent joint-VJP diagnostics")
        if inference_steps != top_level_layers:
            raise RuntimeError(
                "EQ-S2 output does not align FixedPred steps with model layers"
            )
        if run.get("structural_contract_passed") is not True:
            raise RuntimeError("EQ-S2 output failed its structural contract")

    print(f"OK: controlled EQ-S2 {args.device} gate passed with verified provenance")
