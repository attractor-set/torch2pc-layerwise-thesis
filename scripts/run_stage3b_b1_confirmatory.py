#!/usr/bin/env python3
"""Plan or execute one authorized lane of confirmatory Stage 3B B1."""
...
def parse_args() -> argparse.Namespace:
    ...
    if args.reconcile_running and (
        args.operator_acknowledgement != B1_CONFIRMATORY_RECONCILE_ACKNOWLEDGEMENT
    ):
...
def main() -> int:
    ...
    verification = verify_b1_confirmatory_authorization_for_lane(
        authorization,
        request,
    ...
    if not plan.selected_pair_ids:
        print("INFO: no pairs selected for execution")
        return 0
    specs_by_id = {
        spec.pair_id: spec
    ...
    started = {
        ...
        "selected_pair_count": len(plan.selected_pair_ids),
        "selected_pair_ids": list(plan.selected_pair_ids),
        "resume": args.resume,
        "retry_failed": args.retry_failed,
        "engineering_smoke": args.engineering_smoke,
        "evidence": False,
    ...
    try:
        ...
        for pair_id in plan.selected_pair_ids:
            ...
            if emergency_stop.exists():
                raise B1ConfirmatoryOperatorInterruption(
            ...
            completed_count += 1
            print(f"COMPLETED_PAIR={pair_id}")
        completed = {
            **started,
            "status": "completed",
        ...
        return 0
    except (B1ConfirmatoryOperatorInterruption, KeyboardInterrupt) as error:
        failed = {
            **started,
            "status": "failed",
        ...
        return 130
    except B1ConfirmatoryScientificError as error:
        failed = {
            **started,
            "status": "failed",
        ...
        print(f"ERROR: {error}")
        return 3
    except (B1ConfirmatoryError, OSError) as error:
        failed = {
            **started,
            "status": "failed",
...
def _acquire_lock(path: Path, run_id: str) -> int:
    try:
        ...
        raise B1ConfirmatoryError(f"lane lock is active: {path}") from error
    os.write(fd, (run_id + "\n").encode("utf-8"))
    return fd
...
def _write_plan(path: Path, output_root: Path, payload: dict[str, object]) -> None:
    ...
    try:
        ...
        raise B1ConfirmatoryError(f"plan output must be under output root: {resolved}") from error
