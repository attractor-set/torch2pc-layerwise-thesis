#!/usr/bin/env python3
"""Materialize and execute exactly one QWake Attempt-004 engineering run.
...
3. materialize a new Attempt-004 execution freeze;
...
Attempt-004 execution lease is atomically materialized. The host never writes
...
class Attempt004HostOneShotError(RuntimeError):
...
def _sha256_file(path: Path) -> str:
    if not path.is_file() or path.is_symlink():
        raise Attempt004HostOneShotError(f"regular file required: {path}")
...
def _remote_main(root: Path) -> str:
    ...
    if len(parts) != 2 or parts[1] != "refs/heads/main":
        raise Attempt004HostOneShotError("remote main identity differs")
    return parts[0]
...
def _write_exclusive(path: Path, content: bytes, mode: int = 0o600) -> None:
    if not content:
        raise Attempt004HostOneShotError("empty durable payload")
...
def _require_absent_effects(root: Path) -> None:
    ...
    staging = tuple(
        (root / ATTEMPT_004_OUTPUT_ROOT.parent).glob(
            f".{ATTEMPT_004_OUTPUT_ROOT.name}.staging-*"
        )
    )
    if staging:
        raise Attempt004HostOneShotError(
            "Attempt-004 staging already exists"
        )


def _require_source_state(root: Path) -> tuple[str, str]:
    if Path(_git_scalar(root, "rev-parse", "--show-toplevel")).resolve() != root:
        raise Attempt004HostOneShotError("project root differs")
    ...
    if not torch2pc.is_dir() or torch2pc.is_symlink():
        raise Attempt004HostOneShotError("Torch2PC checkout is absent")
    torch2pc_head = _git_scalar(torch2pc, "rev-parse", "HEAD")
    if torch2pc_head != EXPECTED_TORCH2PC_COMMIT:
        raise Attempt004HostOneShotError(
            "Torch2PC commit differs from preregistered Attempt-004 input"
        )
...
def _build_image(
    ...
    if docker is None:
        raise Attempt004HostOneShotError("docker executable is unavailable")
    tag = f"torch2pc-layerwise-thesis:attempt004-{source_commit[:12]}"
    build_argv = [
        docker,
        "build",
        "--file",
    ...
    if not isinstance(labels, dict):
        raise Attempt004HostOneShotError("Docker labels differ")
    if labels.get("org.opencontainers.image.revision") != source_commit:
        raise Attempt004HostOneShotError("image source revision differs")
    if labels.get("io.torch2pc.base-image") != BASE_IMAGE:
        raise Attempt004HostOneShotError("image base-image label differs")
...
def _materialize_freeze(
    ...
    *,
    ...
    source_files = {
        ATTEMPT_004_CONTRACT_RELATIVE.as_posix(): _sha256_file(
            root / ATTEMPT_004_CONTRACT_RELATIVE
        ),
        ATTEMPT_004_WRAPPER_RELATIVE.as_posix(): _sha256_file(
            root / ATTEMPT_004_WRAPPER_RELATIVE
        ),
        ATTEMPT_004_BACKEND_RELATIVE.as_posix(): _sha256_file(
            root / ATTEMPT_004_BACKEND_RELATIVE
        ),
        ATTEMPT_004_PROFILE_RELATIVE.as_posix(): _sha256_file(
            root / ATTEMPT_004_PROFILE_RELATIVE
        ),
        GENERIC_RUNTIME_BACKEND_RELATIVE.as_posix(): _sha256_file(
            root / GENERIC_RUNTIME_BACKEND_RELATIVE
        ),
        ATTEMPT_004_HOST_SPAWNER_RELATIVE.as_posix(): _sha256_file(
            root / ATTEMPT_004_HOST_SPAWNER_RELATIVE
        ),
        ATTEMPT_004_ENTRYPOINT_RELATIVE.as_posix(): _sha256_file(
            root / ATTEMPT_004_ENTRYPOINT_RELATIVE
        ),
    ...
    freeze_payload: dict[str, object] = {
        ...
        "contract_sha256": source_files[
            ATTEMPT_004_CONTRACT_RELATIVE.as_posix()
        ],
        "wrapper_sha256": source_files[
            ATTEMPT_004_WRAPPER_RELATIVE.as_posix()
        ],
        "backend_sha256": source_files[
            ATTEMPT_004_BACKEND_RELATIVE.as_posix()
        ],
        "profile_sha256": source_files[
            ATTEMPT_004_PROFILE_RELATIVE.as_posix()
        ],
        "generic_backend_sha256": source_files[
            GENERIC_RUNTIME_BACKEND_RELATIVE.as_posix()
        ],
        "host_spawner_sha256": source_files[
            ATTEMPT_004_HOST_SPAWNER_RELATIVE.as_posix()
        ],
        "entrypoint_sha256": source_files[
            ATTEMPT_004_ENTRYPOINT_RELATIVE.as_posix()
        ],
    ...
    freeze = Attempt004ExecutionFreeze(**cast(dict[str, Any], freeze_payload))
    freeze.require()

    freeze_root = root / ATTEMPT_004_FREEZE_ROOT
    ...
    verified = verify_attempt_004_execution_freeze(
        root,
        require_runtime_environment=False,
    )
    if verified != freeze:
        raise Attempt004HostOneShotError(
            "materialized Attempt-004 freeze differs"
        )
    return freeze
...
def _materialize_authorization(
    ...
    *,
    ...
    if action_phrase != ATTEMPT_004_INVOCATION_ACKNOWLEDGEMENT:
        raise Attempt004HostOneShotError(
            "Attempt-004 authorization phrase differs"
        )
    ...
    authorization = Attempt004Authorization(**cast(dict[str, Any], payload))
    authorization.require(freeze)

    authorization_root = root / ATTEMPT_004_AUTHORIZATION_ROOT
    ...
    source_relatives = [
        ATTEMPT_004_FREEZE_RELATIVE.as_posix(),
        (ATTEMPT_004_FREEZE_ROOT / "SHA256SUMS").as_posix(),
        (ATTEMPT_004_FREEZE_ROOT / "source-SHA256SUMS").as_posix(),
        ATTEMPT_004_CONTRACT_RELATIVE.as_posix(),
        ATTEMPT_004_HOST_SPAWNER_RELATIVE.as_posix(),
    ]
    _write_registry(
    ...
    verified = verify_unconsumed_attempt_004_authorization(root, freeze)
    if verified != authorization:
        raise Attempt004HostOneShotError(
            "materialized Attempt-004 authorization differs"
        )
...
def _materialize_command(
    ...
    *,
    ...
    if lease_acknowledgement != ATTEMPT_004_LEASE_ACKNOWLEDGEMENT:
        raise Attempt004HostOneShotError(
    ...
    for value, label in (
        ...
        if re.fullmatch(r"[1-9][0-9]*(?:[kKmMgG])?", value) is None:
            raise Attempt004HostOneShotError(f"{label} differs")
    ...
    payload: dict[str, object] = {
        ...
        "host_command_constructor": ATTEMPT_004_HOST_SPAWNER_RELATIVE.as_posix(),
    ...
    _write_exclusive(
        root / ATTEMPT_004_HOST_COMMAND_RELATIVE,
...
def _run_one_child(
    ...
    outcome_path = root / ATTEMPT_004_DURABLE_OUTCOME_RELATIVE
    ...
    lease_exists = (
        root / ATTEMPT_004_LEASE_V1_RELATIVE
    ).is_file()
    output_exists = (root / ATTEMPT_004_OUTPUT_ROOT).is_dir()
    ...
    print("=== ATTEMPT-004 HOST ONE-SHOT OUTCOME ===")
    ...
    if return_code is None:
        raise Attempt004HostOneShotError("host child return code is absent")
    return return_code
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        required=True,
        help=f"Must exactly equal {ATTEMPT_004_INVOCATION_ACKNOWLEDGEMENT}",
    )
    parser.add_argument(
        "--lease-acknowledgement",
        required=True,
        help=f"Must exactly equal {ATTEMPT_004_LEASE_ACKNOWLEDGEMENT}",
    )
...
def main() -> int:
    ...
    print("=== ATTEMPT-004 PRE-SPAWN IDENTITY ===")
    ...
    if return_code != 0:
        raise Attempt004HostOneShotError(
            f"Attempt-004 child returned {return_code}; no retry is permitted"
        )
