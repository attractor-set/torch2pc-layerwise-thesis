...
_ALLOWED_TRACKS = {"baseline", "implementation_preserving", "approximation"}
...
def validate_stage3_design(design: Stage3Design) -> None:
    ...
    execution = _full_commit(baseline.get("stage2_execution_source"), "baseline.stage2_execution_source")
    ...
    for index, raw in enumerate(candidates):
        ...
        if track == "implementation_preserving" and changes_algorithm:
            raise Stage3DesignError(
                f"implementation-preserving candidate {candidate_id} cannot change the algorithm"
            )
        if track == "approximation" and not changes_algorithm:
            raise Stage3DesignError(
                f"approximation candidate {candidate_id} must be marked algorithm-changing"
            )
        if track == "implementation_preserving" and candidate.get("equivalence_gate") is not True:
            raise Stage3DesignError(
                f"implementation-preserving candidate {candidate_id} requires equivalence_gate=true"
            )
        ...
        methods = {
            str(value)
            for value in _sequence(candidate.get("methods"), f"{candidate_id}.methods")
        }
    ...
    for phase_name in ["profiling", "pilot"]:
    ...
    pilot = _mapping(phases["pilot"], "phases.pilot")
    pilot_candidates = [str(value) for value in _sequence(pilot.get("candidates"), "pilot.candidates")]
    unknown = sorted(set(pilot_candidates) - set(candidate_by_id))
    if unknown:
        raise Stage3DesignError(f"pilot references unknown candidates: {unknown}")
    deferred = [
        candidate_id
    ...
    if deferred:
        raise Stage3DesignError(f"pilot cannot include deferred candidates: {deferred}")

    methods = {str(value) for value in _sequence(pilot.get("methods"), "pilot.methods")}
    if not methods <= _ALLOWED_METHODS:
        raise Stage3DesignError("pilot contains unsupported methods")
    seeds = [int(value) for value in _sequence(pilot.get("seeds"), "pilot.seeds")]
    if len(seeds) != len(set(seeds)) or len(seeds) < 3:
        raise Stage3DesignError("pilot.seeds must contain at least three unique values")

    gates = _mapping(design.get("gates"), "gates")
    for gate in ["equivalence", "non_inferiority", "performance", "locality"]:
...
def build_stage3_design_plan(design: Stage3Design) -> dict[str, Any]:
    ...
    pilot_order_seed = int(pilot["execution_order_seed"])
    pilot_cells.sort(
        key=lambda cell: hashlib.sha256(
            f"{pilot_order_seed}|{cell['dataset']}|{cell['model']}|{cell['method']}|"
...
def stage3_readiness_report(
    ...
    blockers = [
        ...
        "Stage 3 CPU/GPU equivalence and approximation gates have not been produced yet.",
        "Stage 3 profiling and pilot environment locks have not been frozen yet.",
