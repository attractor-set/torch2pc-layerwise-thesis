...
ATTEMPT_003_ID: Final = "stage3b-qwake-lc4-runtime-validation-v1-attempt-003"
ATTEMPT_003_OUTPUT_ROOT: Final = Path(
    "results/stage-3/qwake-lc4-runtime-validation-v1-attempt-003"
)
ATTEMPT_003_LEASE_V1_RELATIVE: Final = Path(
    str(ATTEMPT_003_OUTPUT_ROOT) + ".execution-lease.json"
)
ATTEMPT_003_LEASE_V2_RELATIVE: Final = Path(
    str(ATTEMPT_003_OUTPUT_ROOT) + ".execution-lease-v2.json"
)
ATTEMPT_003_DURABLE_OUTCOME_RELATIVE: Final = Path(
    str(ATTEMPT_003_OUTPUT_ROOT) + ".host-outcome.json"
)
ATTEMPT_003_FREEZE_ROOT: Final = Path(
    "experiments/frozen/stage3b-qwake-attempt-003-execution-freeze-v1"
)
ATTEMPT_003_FREEZE_RELATIVE: Final = ATTEMPT_003_FREEZE_ROOT / "execution.json"
ATTEMPT_003_AUTHORIZATION_ROOT: Final = Path(
    "experiments/frozen/stage3b-qwake-attempt-003-authorization-v1"
)
ATTEMPT_003_AUTHORIZATION_RELATIVE: Final = (
    ATTEMPT_003_AUTHORIZATION_ROOT / "authorization.json"
)
...
ATTEMPT_003_CONTRACT_RELATIVE: Final = Path(
    "src/torch2pc_thesis/stage3b_qwake_attempt_003_contract.py"
)
ATTEMPT_003_WRAPPER_RELATIVE: Final = Path(
    "src/torch2pc_thesis/stage3b_qwake_attempt_003_execution_wrapper.py"
)
ATTEMPT_003_BACKEND_RELATIVE: Final = Path(
    "src/torch2pc_thesis/stage3b_qwake_attempt_003_runtime_backend.py"
)
ATTEMPT_003_ENTRYPOINT_RELATIVE: Final = Path(
    "scripts/run_stage3b_qwake_attempt_003_authorized_runtime.py"
)
ATTEMPT_003_LEASE_ACKNOWLEDGEMENT: Final = (
    "CLAIM_QWAKE_LC4_ATTEMPT_003_FROM_CORRECTED_EXECUTION_FREEZE"
)
ATTEMPT_003_INVOCATION_ACKNOWLEDGEMENT: Final = (
    "AUTHORIZE_QWAKE_LC4_ATTEMPT_003_ONE_SHOT_ENGINEERING_INVOCATION"
)
ATTEMPT_003_FREEZE_ID: Final = (
    "stage3b-qwake-attempt-003-execution-freeze-v1"
)
ATTEMPT_003_FREEZE_STATUS: Final = (
    "corrected_image_and_attempt_003_runtime_frozen_execution_not_started"
)
ATTEMPT_003_AUTHORIZATION_ID: Final = (
    "stage3b-qwake-attempt-003-authorization-v1"
)
ATTEMPT_003_AUTHORIZATION_STATUS: Final = (
    "effective_unconsumed_attempt_003_runtime_authorization"
)
...
__all__ = [
    ...
    "ATTEMPT_003_AUTHORIZATION_RELATIVE",
...
class Attempt003ContractError(RuntimeError):
    """Raised when a future attempt-003 identity fails closed."""


@dataclass(frozen=True)
class Attempt003ExecutionFreeze:
    """Exact corrected image and source identities for attempt 003."""
    ...
    def require(self) -> None:
        expected: Mapping[str, object] = {
            ...
            "durable_outcome_relative": (
                ATTEMPT_003_DURABLE_OUTCOME_RELATIVE.as_posix()
            ),
        ...
        for field_name, expected_value in expected.items():
            if getattr(self, field_name) != expected_value:
                raise Attempt003ContractError(
                    f"attempt-003 freeze {field_name} differs"
                )
        ...
        if self.image_repo_digest.count("@sha256:") != 1:
            raise Attempt003ContractError("attempt-003 image repo digest differs")
        image_hex = self.image_digest.removeprefix("sha256:")
        if not self.image_repo_digest.endswith(image_hex):
            raise Attempt003ContractError(
                "attempt-003 image and repository digests differ"
            )
        if self.freeze_sha256 != sha256_object(self._payload_without_digest()):
            raise Attempt003ContractError("attempt-003 freeze digest differs")
...
class Attempt003Authorization:
    ...
    def require(self, freeze: Attempt003ExecutionFreeze) -> None:
        freeze.require()
        expected: Mapping[str, object] = {
            "schema_version": 1,
        ...
        for field_name, expected_value in expected.items():
            if getattr(self, field_name) != expected_value:
                raise Attempt003ContractError(
                    f"attempt-003 authorization {field_name} differs"
                )
        if not self.operator_identity:
            raise Attempt003ContractError(
                "attempt-003 authorization operator identity is empty"
            )
        ...
        if self.authorization_sha256 != sha256_object(
            ...
            raise Attempt003ContractError(
                "attempt-003 authorization digest differs"
            )
...
@dataclass(frozen=True)
class Attempt003AdmissionIdentity:
    ...
    def require(self) -> None:
        ...
        for field_name, expected_value in expected.items():
            if getattr(self, field_name) != expected_value:
                raise Attempt003ContractError(
                    f"attempt-003 admission {field_name} differs"
                )
        for value, field_name in (
            (self.freeze_sha256, "freeze_sha256"),
        ...
        for value, field_name in (
            ...
            _require_commit(value, field_name)
        if self.image_repo_digest.count("@sha256:") != 1:
            raise Attempt003ContractError("attempt-003 admission image differs")
...
def build_attempt_003_admission(
...
def verify_attempt_003_execution_freeze(
    project_root: Path,
) -> Attempt003ExecutionFreeze:
    """Load and verify the future corrected image freeze without effects."""

    root = project_root.expanduser().resolve()
    freeze_path = root / ATTEMPT_003_FREEZE_RELATIVE
    mapping = _read_json_object(freeze_path)
    freeze = Attempt003ExecutionFreeze(**cast(dict[str, Any], mapping))
    freeze.require()

    expected_files = {
        ATTEMPT_003_CONTRACT_RELATIVE: freeze.contract_sha256,
        ATTEMPT_003_WRAPPER_RELATIVE: freeze.wrapper_sha256,
        ATTEMPT_003_BACKEND_RELATIVE: freeze.backend_sha256,
        ATTEMPT_003_ENTRYPOINT_RELATIVE: freeze.entrypoint_sha256,
        SCIENTIFIC_RUNTIME_AUTHORIZATION_RELATIVE: (
    ...
    for relative, expected_sha256 in expected_files.items():
        if _sha256_file(root / relative) != expected_sha256:
            raise Attempt003ContractError(
                f"attempt-003 frozen source differs: {relative}"
            )

    _verify_registry(
        root / ATTEMPT_003_FREEZE_ROOT / "SHA256SUMS",
        root / ATTEMPT_003_FREEZE_ROOT,
    )
    _verify_registry(
        root / ATTEMPT_003_FREEZE_ROOT / "source-SHA256SUMS",
        root,
    )
    ...
    for name, expected in environment.items():
        if os.environ.get(name) != expected:
            raise Attempt003ContractError(
                f"attempt-003 runtime environment {name} differs"
            )
    return freeze


def verify_unconsumed_attempt_003_authorization(
    project_root: Path,
    freeze: Attempt003ExecutionFreeze,
) -> Attempt003Authorization:
    ...
    root = project_root.expanduser().resolve()
    freeze.require()
    mapping = _read_json_object(root / ATTEMPT_003_AUTHORIZATION_RELATIVE)
    authorization = Attempt003Authorization(**cast(dict[str, Any], mapping))
    authorization.require(freeze)
    _verify_registry(
        root / ATTEMPT_003_AUTHORIZATION_ROOT / "SHA256SUMS",
        root / ATTEMPT_003_AUTHORIZATION_ROOT,
    )
    _verify_registry(
        root / ATTEMPT_003_AUTHORIZATION_ROOT / "source-SHA256SUMS",
        root,
    )
    ...
    if tuple((root / ATTEMPT_003_OUTPUT_ROOT.parent).glob(staging_pattern)):
        raise Attempt003ContractError("attempt-003 staging already exists")
    return authorization


def _read_json_object(path: Path) -> dict[str, object]:
    if not path.is_file() or path.is_symlink():
        raise Attempt003ContractError(f"regular JSON file is absent: {path}")
    try:
        value = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise Attempt003ContractError(f"invalid JSON file: {path}") from exc
    if not isinstance(value, dict):
        raise Attempt003ContractError(f"JSON root is not an object: {path}")
    if path.read_bytes() != canonical_json(value).encode("utf-8"):
        raise Attempt003ContractError(f"JSON serialization differs: {path}")
...
def _verify_registry(registry_path: Path, base: Path) -> None:
    ...
    for raw_line in registry_path.read_text(encoding="utf-8").splitlines():
        ...
        if len(parts) != 2:
            raise Attempt003ContractError("registry line shape differs")
        expected, relative = parts
        _require_hex_sha256(expected, "registry digest")
        if relative in seen:
            raise Attempt003ContractError("registry path is duplicated")
        seen.add(relative)
        candidate = (base / relative).resolve()
        try:
            candidate.relative_to(base.resolve())
        except ValueError as exc:
            raise Attempt003ContractError("registry path escapes base") from exc
        if _sha256_file(candidate) != f"sha256:{expected}":
            raise Attempt003ContractError(
                f"registry file identity differs: {relative}"
            )
    if not seen:
        raise Attempt003ContractError("registry is empty")


def _sha256_file(path: Path) -> str:
    if not path.is_file() or path.is_symlink():
        raise Attempt003ContractError(f"regular source file is absent: {path}")
...
def _require_commit(value: str, field_name: str) -> None:
    ...
    if len(value) != 40 or invalid:
        raise Attempt003ContractError(f"{field_name} is not a commit")


def _require_sha256(value: str, field_name: str) -> None:
    if not value.startswith("sha256:") or len(value) != 71:
        raise Attempt003ContractError(f"{field_name} is not SHA-256")
...
def _require_hex_sha256(value: str, field_name: str) -> None:
    ...
    if len(value) != 64 or invalid:
        raise Attempt003ContractError(f"{field_name} is not hexadecimal SHA-256")
