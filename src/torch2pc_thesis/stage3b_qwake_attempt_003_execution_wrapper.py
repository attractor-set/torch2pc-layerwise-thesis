"""Atomic lease and no-replace output wrapper for attempt 003.

All effects are explicit.  The implementation owns only the attempt-003 paths
...
from torch2pc_thesis.stage3b_qwake_attempt_003_contract import (
    ...
    RESERVE_PROBE_COUNT,
    Attempt003AdmissionIdentity,
    canonical_json,
    sha256_object,
)

ATTEMPT_003_LEASE_ID: Final = "stage3b-qwake-attempt-003-lease-v1"
ATTEMPT_003_WRAPPER_CONTRACT_ID: Final = (
    "stage3b-qwake-attempt-003-wrapper-contract-v1"
)
ATTEMPT_003_BACKEND_RECEIPT_ID: Final = (
    "stage3b-qwake-attempt-003-backend-receipt-v1"
)
ATTEMPT_003_WRAPPER_RECEIPT_ID: Final = (
    "stage3b-qwake-attempt-003-wrapper-receipt-v1"
)
ATTEMPT_003_WRAPPER_RECEIPT_RELATIVE: Final = Path(
...
__all__ = [
    "ATTEMPT_003_BACKEND_RECEIPT_ID",
    "ATTEMPT_003_LEASE_ID",
    "ATTEMPT_003_WRAPPER_CONTRACT_ID",
    "ATTEMPT_003_WRAPPER_RECEIPT_ID",
    "ATTEMPT_003_WRAPPER_RECEIPT_RELATIVE",
    "Attempt003BackendReceipt",
    "Attempt003ExecutionBackend",
    "Attempt003ExecutionOutcome",
    "Attempt003ExecutionWrapperError",
    "Attempt003LeaseV1",
    "Attempt003WrapperContract",
    "build_attempt_003_backend_receipt",
    "build_attempt_003_lease",
    "build_attempt_003_wrapper_contract",
    "load_attempt_003_lease",
    "materialize_attempt_003_lease",
    "run_claimed_attempt_003",
]


class Attempt003ExecutionWrapperError(RuntimeError):
    """Raised when attempt-003 effects cannot preserve the contract."""


@dataclass(frozen=True)
class Attempt003LeaseV1:
    """Canonical one-shot claim for attempt 003."""
    ...
    def require(self, admission: Attempt003AdmissionIdentity) -> None:
        admission.require()
        expected: Mapping[str, object] = {
            "schema_version": 1,
            "lease_id": ATTEMPT_003_LEASE_ID,
            ...
            "operator_acknowledgement": ATTEMPT_003_LEASE_ACKNOWLEDGEMENT,
            "output_root": ATTEMPT_003_OUTPUT_ROOT.as_posix(),
            "execution_lease_relative": (
                ATTEMPT_003_LEASE_V1_RELATIVE.as_posix()
            ),
        ...
        for field_name, expected_value in expected.items():
            if getattr(self, field_name) != expected_value:
                raise Attempt003ExecutionWrapperError(
                    f"attempt-003 lease {field_name} differs"
                )
        ...
        if self.lease_sha256 != sha256_object(self._payload_without_digest()):
            raise Attempt003ExecutionWrapperError(
                "attempt-003 lease digest differs"
            )
...
@dataclass(frozen=True)
class Attempt003WrapperContract:
    ...
    def require(self, lease: Attempt003LeaseV1) -> None:
        expected: Mapping[str, object] = {
            "schema_version": 1,
            "contract_id": ATTEMPT_003_WRAPPER_CONTRACT_ID,
            "attempt_id": ATTEMPT_003_ID,
        ...
        for field_name, expected_value in expected.items():
            if getattr(self, field_name) != expected_value:
                raise Attempt003ExecutionWrapperError(
                    f"attempt-003 wrapper contract {field_name} differs"
                )
        ...
        if self.contract_sha256 != sha256_object(
            ...
            raise Attempt003ExecutionWrapperError(
                "attempt-003 wrapper contract digest differs"
            )
...
@dataclass(frozen=True)
class Attempt003BackendReceipt:
    """Canonical receipt returned by an injected attempt-003 backend."""
    ...
    def require(
        self,
        lease: Attempt003LeaseV1,
        contract: Attempt003WrapperContract,
    ) -> None:
        contract.require(lease)
        expected: Mapping[str, object] = {
            "schema_version": 1,
            "receipt_id": ATTEMPT_003_BACKEND_RECEIPT_ID,
            "attempt_id": ATTEMPT_003_ID,
            "freeze_sha256": lease.freeze_sha256,
            "authorization_sha256": lease.authorization_sha256,
            "lease_sha256": lease.lease_sha256,
        ...
        for field_name, expected_value in expected.items():
            if getattr(self, field_name) != expected_value:
                raise Attempt003ExecutionWrapperError(
                    f"attempt-003 backend receipt {field_name} differs"
                )
        if not self.backend_id:
            raise Attempt003ExecutionWrapperError(
                "attempt-003 backend id is empty"
            )
        if self.output_file_count < 1:
            raise Attempt003ExecutionWrapperError(
                "attempt-003 backend produced no regular file"
            )
        ...
        if self.receipt_sha256 != sha256_object(
            ...
            raise Attempt003ExecutionWrapperError(
                "attempt-003 backend receipt digest differs"
            )
...
class Attempt003ExecutionBackend(Protocol):
...
@dataclass(frozen=True)
class Attempt003ExecutionOutcome:
    """Successful no-replace promotion for attempt 003."""

    lease_path: Path
    output_root: Path
    wrapper_receipt_sha256: str
    backend_receipt: Attempt003BackendReceipt

    def require(self) -> None:
        if not self.lease_path.is_file() or self.lease_path.is_symlink():
            raise Attempt003ExecutionWrapperError(
                "attempt-003 outcome lease is absent"
            )
        if not self.output_root.is_dir() or self.output_root.is_symlink():
            raise Attempt003ExecutionWrapperError(
                "attempt-003 outcome root is absent"
            )
        _require_sha256(
...
def build_attempt_003_lease(
    admission: Attempt003AdmissionIdentity,
    *,
    claimed_at_utc: str,
    wrapper_commit: str,
    operator_acknowledgement: str,
    ...
    payload: dict[str, object] = {
        ...
        "output_root": ATTEMPT_003_OUTPUT_ROOT.as_posix(),
        "execution_lease_relative": ATTEMPT_003_LEASE_V1_RELATIVE.as_posix(),
        "output_root_absent_at_claim": True,
        "execution_lease_absent_at_claim": True,
        "execution_count": 1,
        "authorization_consumed": True,
...
def build_attempt_003_wrapper_contract(
    lease: Attempt003LeaseV1,
    admission: Attempt003AdmissionIdentity,
) -> Attempt003WrapperContract:
...
def build_attempt_003_backend_receipt(
    *,
    backend_id: str,
    lease: Attempt003LeaseV1,
    contract: Attempt003WrapperContract,
    ...
    receipt = Attempt003BackendReceipt(
        **cast(dict[str, Any], payload),
...
def materialize_attempt_003_lease(
    project_root: Path,
    lease: Attempt003LeaseV1,
    admission: Attempt003AdmissionIdentity,
) -> Path:
    ...
    lease.require(admission)
    output_root = root / ATTEMPT_003_OUTPUT_ROOT
    lease_path = root / ATTEMPT_003_LEASE_V1_RELATIVE
    if _lexists(output_root):
        raise Attempt003ExecutionWrapperError(
            "attempt-003 output root already exists"
        )
    if _lexists(lease_path):
        raise Attempt003ExecutionWrapperError(
            "attempt-003 execution lease already exists"
        )
    ...
    if _lexists(output_root):
        raise Attempt003ExecutionWrapperError(
            "attempt-003 output appeared during lease claim"
        )
    observed = load_attempt_003_lease(
        root,
        admission,
    ...
    if observed != lease:
        raise Attempt003ExecutionWrapperError(
            "materialized attempt-003 lease differs"
        )
    return lease_path


def load_attempt_003_lease(
    project_root: Path,
    admission: Attempt003AdmissionIdentity,
    *,
    ...
    if not lease_path.is_file() or lease_path.is_symlink():
        raise Attempt003ExecutionWrapperError(
            "attempt-003 execution lease is absent or non-regular"
        )
    try:
        value = json.loads(lease_path.read_text(encoding="utf-8"))
    except (OSError, UnicodeDecodeError, ValueError) as exc:
        raise Attempt003ExecutionWrapperError(
            "attempt-003 execution lease JSON is invalid"
        ) from exc
    if not isinstance(value, dict):
        raise Attempt003ExecutionWrapperError(
            "attempt-003 execution lease root differs"
        )
    lease = Attempt003LeaseV1(**cast(dict[str, Any], value))
    lease.require(admission)
    if stat.S_IMODE(lease_path.stat(follow_symlinks=False).st_mode) != 0o600:
        raise Attempt003ExecutionWrapperError(
            "attempt-003 execution lease mode differs"
        )
    if lease_path.read_bytes() != lease.canonical_json().encode("utf-8"):
        raise Attempt003ExecutionWrapperError(
            "attempt-003 execution lease serialization differs"
        )
    output_root = root / ATTEMPT_003_OUTPUT_ROOT
...
def run_claimed_attempt_003(
    project_root: Path,
    admission: Attempt003AdmissionIdentity,
    lease: Attempt003LeaseV1,
    *,
    backend: Attempt003ExecutionBackend,
) -> Attempt003ExecutionOutcome:
    ...
    if observed != lease:
        raise Attempt003ExecutionWrapperError(
            "attempt-003 provided lease differs from materialized claim"
        )
    contract = build_attempt_003_wrapper_contract(lease, admission)
    if not backend.backend_id:
        raise Attempt003ExecutionWrapperError("attempt-003 backend id is empty")
...
def _atomic_exclusive_file_claim(root: Path, target: Path, payload: bytes) -> None:
    if not payload:
        raise Attempt003ExecutionWrapperError("atomic file payload is empty")
...
def _rename_noreplace(source: Path, target: Path) -> None:
    ...
    try:
        ...
        raise Attempt003ExecutionWrapperError("renameat2 is unavailable") from exc
    renameat2.argtypes = [
        ctypes.c_int,
        ctypes.c_char_p,
        ctypes.c_int,
...
def _validate_staging_tree(staging_root: Path) -> int:
    if not staging_root.is_dir() or staging_root.is_symlink():
        raise Attempt003ExecutionWrapperError(
            "attempt-003 staging root is absent or non-directory"
        )
    ...
    for current_root, directory_names, file_names in os.walk(
        ...
        for directory_name in directory_names:
            ...
            if directory.is_symlink() or not directory.is_dir():
                raise Attempt003ExecutionWrapperError(
                    "attempt-003 staging contains a non-directory entry"
                )
        for file_name in file_names:
            file_path = current / file_name
            if file_path.is_symlink() or not file_path.is_file():
                raise Attempt003ExecutionWrapperError(
                    "attempt-003 staging contains a non-regular file"
                )
            regular_file_count += 1
    if regular_file_count < 1:
        raise Attempt003ExecutionWrapperError(
            "attempt-003 staging contains no regular output file"
        )
    ...
    for current_root, directory_names, file_names in os.walk(
        ...
        for directory_name in directory_names:
            ...
            if directory.is_symlink():
                raise Attempt003ExecutionWrapperError(
                    "attempt-003 staging contains a symlink"
                )
...
def _require_commit(value: str, field_name: str) -> None:
    ...
    if len(value) != 40 or invalid:
        raise Attempt003ExecutionWrapperError(f"{field_name} is not a commit")


def _require_sha256(value: str, field_name: str) -> None:
    if not value.startswith("sha256:") or len(value) != 71:
        raise Attempt003ExecutionWrapperError(f"{field_name} is not SHA-256")
...
def _require_utc(value: str, field_name: str) -> None:
    if not value.endswith("Z"):
        raise Attempt003ExecutionWrapperError(f"{field_name} is not UTC")
    try:
        parsed = datetime.fromisoformat(value.replace("Z", "+00:00"))
    except ValueError as exc:
        raise Attempt003ExecutionWrapperError(f"{field_name} is invalid") from exc
