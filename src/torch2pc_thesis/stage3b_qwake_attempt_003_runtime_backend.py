"""Corrected bounded runtime backend for attempt 003.

The module reuses only the immutable scientific matrix executor.  It owns new
attempt-003 output identities and verifies the new corrected freeze before any
matrix execution.
"""
...
ATTEMPT_003_BACKEND_ID: Final = (
    "stage3b-qwake-attempt-003-bounded-torch-runtime-backend-v1"
)
ATTEMPT_003_REPORT_ID: Final = (
    "stage3b-qwake-attempt-003-runtime-backend-report-v1"
)
...
__all__ = [
    "ATTEMPT_003_BACKEND_ID",
    "ATTEMPT_003_REPORT_ID",
    "Attempt003RuntimeBackend",
    "Attempt003RuntimeBackendError",
]


class Attempt003RuntimeBackendError(RuntimeError):
    """Raised when the attempt-003 backend cannot preserve its freeze."""


class Attempt003RuntimeBackend:
    ...
    def __init__(
        ...
        *,
        project_root: Path,
        torch2pc_dir: Path,
        execution_freeze: Attempt003ExecutionFreeze,
    ...
    def run(
        ...
        if (
            ...
            raise Attempt003RuntimeBackendError(
                "attempt-003 scientific authorization differs"
            )
        if (
            authorization.authorization_sha256
            ...
            raise Attempt003RuntimeBackendError(
                "attempt-003 lease scientific authorization differs"
            )
        ...
        aggregates_bytes = canonical_json(
            {
                "schema_version": 1,
                "aggregate_id": (
                    "stage3b-qwake-attempt-003-paired-cost-aggregates-v1"
                ),
        ...
        report_payload: dict[str, object] = {
            "schema_version": 1,
            "report_id": ATTEMPT_003_REPORT_ID,
            "attempt_id": ATTEMPT_003_ID,
            "status": (
        ...
        receipt = build_attempt_003_backend_receipt(
            backend_id=self.backend_id,
            lease=lease,
            contract=contract,
        ...
        if observed_count != _EXPECTED_OUTPUT_FILE_COUNT:
            raise Attempt003RuntimeBackendError(
                "attempt-003 backend output file count differs"
            )
        return receipt
...
def _write_exclusive_regular(path: Path, content: bytes) -> None:
    if not content:
        raise Attempt003RuntimeBackendError(
            f"attempt-003 output content is empty: {path.name}"
        )
    ...
    try:
        ...
        while view:
            ...
            if written <= 0:
                raise Attempt003RuntimeBackendError(
                    "attempt-003 output write made no progress"
                )
            view = view[written:]
        os.fsync(descriptor)
    finally:
