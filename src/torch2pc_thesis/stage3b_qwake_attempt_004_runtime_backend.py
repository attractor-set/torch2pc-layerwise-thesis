"""Corrected bounded runtime backend for attempt 004.

The module reuses only the immutable scientific matrix executor.  It owns new
attempt-004 output identities and verifies the new corrected freeze before any
matrix execution.
"""
...
from torch2pc_thesis.stage3b_qwake_attempt_004_cpu_measurement_stabilization import (
    ...
    attempt004_cpu_measurement_evidence,
)
...
ATTEMPT_004_BACKEND_ID: Final = (
    "stage3b-qwake-attempt-004-bounded-torch-runtime-backend-v1"
)
ATTEMPT_004_REPORT_ID: Final = (
    "stage3b-qwake-attempt-004-runtime-backend-report-v1"
)
...
__all__ = [
    "ATTEMPT_004_BACKEND_ID",
    "ATTEMPT_004_REPORT_ID",
    "Attempt004RuntimeBackend",
    "Attempt004RuntimeBackendError",
]


class Attempt004RuntimeBackendError(RuntimeError):
    """Raised when the attempt-004 backend cannot preserve its freeze."""


class Attempt004RuntimeBackend:
    ...
    def __init__(
        ...
        *,
        project_root: Path,
        torch2pc_dir: Path,
        execution_freeze: Attempt004ExecutionFreeze,
    ...
    def run(
        ...
        if (
            ...
            raise Attempt004RuntimeBackendError(
                "attempt-004 scientific authorization differs"
            )
        if (
            authorization.authorization_sha256
            ...
            raise Attempt004RuntimeBackendError(
                "attempt-004 lease scientific authorization differs"
            )
        ...
        aggregates_bytes = canonical_json(
            {
                "schema_version": 1,
                "aggregate_id": (
                    "stage3b-qwake-attempt-004-paired-cost-aggregates-v1"
                ),
        ...
        identities_payload: dict[str, object] = {
            ...
            "cpu_measurement_stabilization": attempt004_cpu_measurement_evidence(),
        }
        report_payload: dict[str, object] = {
            "schema_version": 1,
            "report_id": ATTEMPT_004_REPORT_ID,
            "attempt_id": ATTEMPT_004_ID,
            "status": (
        ...
        receipt = build_attempt_004_backend_receipt(
            backend_id=self.backend_id,
            lease=lease,
            contract=contract,
        ...
        if observed_count != _EXPECTED_OUTPUT_FILE_COUNT:
            raise Attempt004RuntimeBackendError(
                "attempt-004 backend output file count differs"
            )
        return receipt
...
def _write_exclusive_regular(path: Path, content: bytes) -> None:
    if not content:
        raise Attempt004RuntimeBackendError(
            f"attempt-004 output content is empty: {path.name}"
        )
    ...
    try:
        ...
        while view:
            ...
            if written <= 0:
                raise Attempt004RuntimeBackendError(
                    "attempt-004 output write made no progress"
                )
            view = view[written:]
        os.fsync(descriptor)
    finally:
