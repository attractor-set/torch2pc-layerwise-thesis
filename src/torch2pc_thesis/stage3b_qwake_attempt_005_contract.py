"""Attempt-005 identities and fail-closed admission verification.
...
ATTEMPT_005_ID: Final = "stage3b-qwake-lc4-runtime-validation-v1-attempt-005"
ATTEMPT_005_OUTPUT_ROOT: Final = Path(
    "results/stage-3/qwake-lc4-runtime-validation-v1-attempt-005"
)
ATTEMPT_005_LEASE_V1_RELATIVE: Final = Path(
    str(ATTEMPT_005_OUTPUT_ROOT) + ".execution-lease.json"
)
ATTEMPT_005_LEASE_V2_RELATIVE: Final = Path(
    str(ATTEMPT_005_OUTPUT_ROOT) + ".execution-lease-v2.json"
)
ATTEMPT_005_DURABLE_OUTCOME_RELATIVE: Final = Path(
    str(ATTEMPT_005_OUTPUT_ROOT) + ".host-outcome.json"
)
ATTEMPT_005_FREEZE_ROOT: Final = Path(
    "experiments/frozen/stage3b-qwake-attempt-005-execution-freeze-v1"
)
ATTEMPT_005_FREEZE_RELATIVE: Final = ATTEMPT_005_FREEZE_ROOT / "execution.json"
ATTEMPT_005_AUTHORIZATION_ROOT: Final = Path(
    "experiments/frozen/stage3b-qwake-attempt-005-authorization-v1"
)
ATTEMPT_005_AUTHORIZATION_RELATIVE: Final = (
    ATTEMPT_005_AUTHORIZATION_ROOT / "authorization.json"
)
...
ATTEMPT_005_CONTRACT_RELATIVE: Final = Path(
    "src/torch2pc_thesis/stage3b_qwake_attempt_005_contract.py"
)
ATTEMPT_005_WRAPPER_RELATIVE: Final = Path(
    "src/torch2pc_thesis/stage3b_qwake_attempt_005_execution_wrapper.py"
)
ATTEMPT_005_BACKEND_RELATIVE: Final = Path(
    "src/torch2pc_thesis/stage3b_qwake_attempt_005_runtime_backend.py"
)
ATTEMPT_005_PROFILE_RELATIVE: Final = Path(
...
ATTEMPT_005_HOST_SPAWNER_RELATIVE: Final = Path(
    "scripts/run_stage3b_qwake_attempt_005_host_one_shot.py"
)
ATTEMPT_005_ENTRYPOINT_RELATIVE: Final = Path(
    "scripts/run_stage3b_qwake_attempt_005_authorized_runtime.py"
)
ATTEMPT_005_LEASE_ACKNOWLEDGEMENT: Final = (
    "CLAIM_QWAKE_LC4_ATTEMPT_005_FROM_LANE_ISOLATED_EXECUTION_FREEZE"
)
ATTEMPT_005_INVOCATION_ACKNOWLEDGEMENT: Final = (
    "AUTHORIZE_QWAKE_LC4_ATTEMPT_005_ONE_SHOT_ENGINEERING_INVOCATION"
)
ATTEMPT_005_FREEZE_ID: Final = (
    "stage3b-qwake-attempt-005-execution-freeze-v1"
)
ATTEMPT_005_FREEZE_STATUS: Final = (
    "lane_isolated_image_and_attempt_005_runtime_frozen_execution_not_started"
)
ATTEMPT_005_AUTHORIZATION_ID: Final = (
    "stage3b-qwake-attempt-005-authorization-v1"
)
ATTEMPT_005_AUTHORIZATION_STATUS: Final = (
    "effective_unconsumed_attempt_005_runtime_authorization"
)
...
ATTEMPT_005_HOST_COMMAND_RELATIVE: Final = Path(
    str(ATTEMPT_005_OUTPUT_ROOT) + ".host-command.json"
)

__all__ = [
    "ATTEMPT_005_AUTHORIZATION_ID",
    "ATTEMPT_005_AUTHORIZATION_RELATIVE",
...
class Attempt005ContractError(RuntimeError):
    """Raised when a future attempt-005 identity fails closed."""


@dataclass(frozen=True)
class Attempt005ExecutionFreeze:
    ...
    def require(self) -> None:
        ...
        for field_name, expected_value in expected.items():
            if getattr(self, field_name) != expected_value:
                raise Attempt005ContractError(
                    f"attempt-005 freeze {field_name} differs"
                )
        ...
        if self.freeze_sha256 != sha256_object(self._payload_without_digest()):
            raise Attempt005ContractError("attempt-005 freeze digest differs")
...
class Attempt005Authorization:
    ...
    def require(self, freeze: Attempt005ExecutionFreeze) -> None:
        freeze.require()
        expected: Mapping[str, object] = {
            "schema_version": 1,
            ...
            "action_phrase": ATTEMPT_005_INVOCATION_ACKNOWLEDGEMENT,
            "execution_count": 1,
        ...
        for field_name, expected_value in expected.items():
            if getattr(self, field_name) != expected_value:
                raise Attempt005ContractError(
                    f"attempt-005 authorization {field_name} differs"
                )
        if not self.operator_identity:
            raise Attempt005ContractError(
                "attempt-005 authorization operator identity is empty"
            )
        ...
        if self.authorization_sha256 != sha256_object(
            ...
            raise Attempt005ContractError(
                "attempt-005 authorization digest differs"
            )
...
@dataclass(frozen=True)
class Attempt005AdmissionIdentity:
    ...
    def require(self) -> None:
        ...
        for field_name, expected_value in expected.items():
            if getattr(self, field_name) != expected_value:
                raise Attempt005ContractError(
                    f"attempt-005 admission {field_name} differs"
                )
        for value, field_name in (
            (self.freeze_sha256, "freeze_sha256"),
...
def _require_optional_repo_digest(value: str, field_name: str) -> None:
    ...
    if re.fullmatch(r"[^@\s]+@sha256:[0-9a-f]{64}", value) is None:
        raise Attempt005ContractError(
            f"attempt-005 {field_name} is not an optional repository digest"
        )
...
def build_attempt_005_admission(
...
def verify_attempt_005_execution_freeze(
    project_root: Path,
    *,
    ...
    freeze_path = root / ATTEMPT_005_FREEZE_RELATIVE
    mapping = _read_json_object(freeze_path)
    freeze = Attempt005ExecutionFreeze(**cast(dict[str, Any], mapping))
    freeze.require()

    expected_files = {
        ATTEMPT_005_CONTRACT_RELATIVE: freeze.contract_sha256,
        ATTEMPT_005_WRAPPER_RELATIVE: freeze.wrapper_sha256,
        ATTEMPT_005_BACKEND_RELATIVE: freeze.backend_sha256,
        ATTEMPT_005_PROFILE_RELATIVE: freeze.profile_sha256,
        GENERIC_RUNTIME_BACKEND_RELATIVE: freeze.generic_backend_sha256,
        ATTEMPT_005_HOST_SPAWNER_RELATIVE: freeze.host_spawner_sha256,
        ATTEMPT_005_ENTRYPOINT_RELATIVE: freeze.entrypoint_sha256,
        SCIENTIFIC_RUNTIME_AUTHORIZATION_RELATIVE: (
    ...
    for relative, expected_sha256 in expected_files.items():
        if _sha256_file(root / relative) != expected_sha256:
            raise Attempt005ContractError(
                f"attempt-005 frozen source differs: {relative}"
            )

    _verify_registry(
        root / ATTEMPT_005_FREEZE_ROOT / "SHA256SUMS",
        root / ATTEMPT_005_FREEZE_ROOT,
    )
    _verify_registry(
        root / ATTEMPT_005_FREEZE_ROOT / "source-SHA256SUMS",
        root,
    )
    ...
    if require_runtime_environment:
        for name, expected in environment.items():
            if os.environ.get(name) != expected:
                raise Attempt005ContractError(
                    f"attempt-005 runtime environment {name} differs"
                )
    return freeze


def verify_unconsumed_attempt_005_authorization(
    project_root: Path,
    freeze: Attempt005ExecutionFreeze,
) -> Attempt005Authorization:
    ...
    root = project_root.expanduser().resolve()
    freeze.require()
    mapping = _read_json_object(root / ATTEMPT_005_AUTHORIZATION_RELATIVE)
    authorization = Attempt005Authorization(**cast(dict[str, Any], mapping))
    authorization.require(freeze)
    _verify_registry(
        root / ATTEMPT_005_AUTHORIZATION_ROOT / "SHA256SUMS",
        root / ATTEMPT_005_AUTHORIZATION_ROOT,
    )
    _verify_registry(
        root / ATTEMPT_005_AUTHORIZATION_ROOT / "source-SHA256SUMS",
        root,
    )
    ...
    staging_pattern = f".{ATTEMPT_005_OUTPUT_ROOT.name}.staging-*"
    if tuple((root / ATTEMPT_005_OUTPUT_ROOT.parent).glob(staging_pattern)):
        raise Attempt005ContractError("attempt-005 staging already exists")
    return authorization


def _read_json_object(path: Path) -> dict[str, object]:
    if not path.is_file() or path.is_symlink():
        raise Attempt005ContractError(f"regular JSON file is absent: {path}")
    try:
        value = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise Attempt005ContractError(f"invalid JSON file: {path}") from exc
    if not isinstance(value, dict):
        raise Attempt005ContractError(f"JSON root is not an object: {path}")
    if path.read_bytes() != canonical_json(value).encode("utf-8"):
        raise Attempt005ContractError(f"JSON serialization differs: {path}")
...
def _verify_registry(registry_path: Path, base: Path) -> None:
    ...
    for raw_line in registry_path.read_text(encoding="utf-8").splitlines():
        ...
        if len(parts) != 2:
            raise Attempt005ContractError("registry line shape differs")
        expected, relative = parts
        _require_hex_sha256(expected, "registry digest")
        if relative in seen:
            raise Attempt005ContractError("registry path is duplicated")
        seen.add(relative)
        candidate = (base / relative).resolve()
        try:
            candidate.relative_to(base.resolve())
        except ValueError as exc:
            raise Attempt005ContractError("registry path escapes base") from exc
        if _sha256_file(candidate) != f"sha256:{expected}":
            raise Attempt005ContractError(
                f"registry file identity differs: {relative}"
            )
    if not seen:
        raise Attempt005ContractError("registry is empty")


def _sha256_file(path: Path) -> str:
    if not path.is_file() or path.is_symlink():
        raise Attempt005ContractError(f"regular source file is absent: {path}")
...
def _require_commit(value: str, field_name: str) -> None:
    ...
    if len(value) != 40 or invalid:
        raise Attempt005ContractError(f"{field_name} is not a commit")


def _require_sha256(value: str, field_name: str) -> None:
    if not value.startswith("sha256:") or len(value) != 71:
        raise Attempt005ContractError(f"{field_name} is not SHA-256")
...
def _require_hex_sha256(value: str, field_name: str) -> None:
    ...
    if len(value) != 64 or invalid:
        raise Attempt005ContractError(f"{field_name} is not hexadecimal SHA-256")
