"""Lane-isolated bounded runtime backend for attempt 005.

The module reuses only the immutable scientific matrix executor.  It owns new
attempt-005 output identities and verifies the new corrected freeze before any
matrix execution.
"""
...
ATTEMPT_005_BACKEND_ID: Final = (
    "stage3b-qwake-attempt-005-bounded-torch-runtime-backend-v1"
)
ATTEMPT_005_REPORT_ID: Final = (
    "stage3b-qwake-attempt-005-runtime-backend-report-v1"
)
...
__all__ = [
    "ATTEMPT_005_BACKEND_ID",
    "ATTEMPT_005_REPORT_ID",
    "Attempt005RuntimeBackend",
    "Attempt005RuntimeBackendError",
]


class Attempt005RuntimeBackendError(RuntimeError):
    """Raised when the attempt-005 backend cannot preserve its freeze."""


class Attempt005RuntimeBackend:
    ...
    def __init__(
        ...
        *,
        project_root: Path,
        torch2pc_dir: Path,
        execution_freeze: Attempt005ExecutionFreeze,
    ...
    def run(
        ...
        if (
            ...
            raise Attempt005RuntimeBackendError(
                "attempt-005 scientific authorization differs"
            )
        if (
            authorization.authorization_sha256
            ...
            raise Attempt005RuntimeBackendError(
                "attempt-005 lease scientific authorization differs"
            )
        ...
        aggregates_bytes = canonical_json(
            {
                "schema_version": 1,
                "aggregate_id": (
                    "stage3b-qwake-attempt-005-paired-cost-aggregates-v1"
                ),
        ...
        identities_payload: dict[str, object] = {
            ...
            "lane_isolation": attempt005_lane_isolation_evidence(),
        }
        report_payload: dict[str, object] = {
            "schema_version": 1,
            "report_id": ATTEMPT_005_REPORT_ID,
            "attempt_id": ATTEMPT_005_ID,
            "status": (
        ...
        receipt = build_attempt_005_backend_receipt(
            backend_id=self.backend_id,
            lease=lease,
            contract=contract,
        ...
        if observed_count != _EXPECTED_OUTPUT_FILE_COUNT:
            raise Attempt005RuntimeBackendError(
                "attempt-005 backend output file count differs"
            )
        return receipt
...
def _write_exclusive_regular(path: Path, content: bytes) -> None:
    if not content:
        raise Attempt005RuntimeBackendError(
            f"attempt-005 output content is empty: {path.name}"
        )
    ...
    try:
        ...
        while view:
            ...
            if written <= 0:
                raise Attempt005RuntimeBackendError(
                    "attempt-005 output write made no progress"
                )
            view = view[written:]
        os.fsync(descriptor)
    finally:
