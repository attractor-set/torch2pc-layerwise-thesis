...
def _require_embedded_runtime_identity(
    ...
    """Bind the running container source closure to the frozen request."""
    ...
    manifest = _resolve_confined(root, _RUNTIME_MANIFEST_RELATIVE.as_posix())
    if not manifest.is_file() or manifest.is_symlink():
        raise ScientificRuntimeError("scientific runtime source manifest is absent")
    if _sha256_file_prefixed(manifest) != request.code_manifest_sha256:
        raise ScientificRuntimeError("scientific runtime source manifest digest differs")
    ...
    for line_number, line in enumerate(
        ...
        seen.add(relative)
        path = _resolve_confined(root, relative)
        if not path.is_file() or path.is_symlink():
            raise ScientificRuntimeError(
                f"runtime source closure path is absent: {relative}"
            )
        if _sha256_file_prefixed(path) != "sha256:" + digest:
            raise ScientificRuntimeError(
    ...
    required = {
        ...
        "scripts/run_stage3b_qwake_scientific_campaign.py",
        "scripts/run_stage3b_qwake_scientific_campaign_host.py",
        ...
        "src/torch2pc_thesis/stage3b_qwake_scientific_runtime.py",
        "src/torch2pc_thesis/data.py",
