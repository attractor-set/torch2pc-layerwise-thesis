...
MODULE_PATH = (
    PROJECT_ROOT
    / "src/torch2pc_thesis/"
    "stage3b_qwake_lc4_final_engineering_invocation_authorization.py"
)
VERIFIER_PATH = (
    PROJECT_ROOT
...
def _authorization():
    return build_final_engineering_invocation_authorization(
        authorization_issued_at_utc="2026-08-03T17:27:00Z",
...
def test_source_packages_and_semantics_are_exact() -> None:
    ...
    assert (
        source.scope_freeze_merge_commit
        == AUTHORIZATION_SCOPE_FREEZE_MERGE_COMMIT
    )
    assert source.scope_freeze_pr_number == 171
    assert source.runtime_entrypoint_module_sha256.startswith("sha256:")


def test_authorization_round_trip_and_validation(tmp_path: Path) -> None:
    ...
    assert authorization.status == FINAL_ENGINEERING_INVOCATION_AUTHORIZATION_STATUS
    assert authorization.operator.identity == AUTHORIZATION_OPERATOR_IDENTITY
    assert authorization.operator.action_phrase == AUTHORIZATION_ACTION_PHRASE
    assert authorization.gates.final_engineering_invocation_authorization_issued is True
    assert (
        authorization.gates.final_engineering_invocation_authorization_post_merge_verified
        is False
    )
    assert authorization.gates.final_engineering_invocation_permitted is False

    path = tmp_path / "authorization.json"
    path.write_text(authorization.canonical_json(), encoding="utf-8")
    loaded = load_final_engineering_invocation_authorization(path)

    validate_final_engineering_invocation_authorization(
        loaded,
        verify_final_engineering_invocation_authorization_sources(PROJECT_ROOT),
        tmp_path,
        expected_authoring_base_commit=AUTHORIZATION_SCOPE_FREEZE_MERGE_COMMIT,
    )
...
def test_semantic_hash_mutation_fails_closed() -> None:
    ...
    with pytest.raises(
        ...
        replace(
            authorization,
            authorization_issued_at_utc="2026-08-03T17:28:00Z",
        ).require()


def test_operator_and_action_phrase_mutation_fail_closed() -> None:
    authorization = _authorization()

    with pytest.raises(FinalEngineeringInvocationAuthorizationError):
        replace(
            authorization,
            operator=replace(
                authorization.operator,
                identity="different-operator",
            ),
        ).require()
    ...
    with pytest.raises(FinalEngineeringInvocationAuthorizationError):
        replace(
            ...
            operator=replace(
                authorization.operator,
                action_phrase="DIFFERENT_ACTION_PHRASE",
            ),
        ).require()


def test_effective_authority_cannot_open_during_authoring() -> None:
    authorization = _authorization()
...
def test_negative_source_mutation_uses_temporary_copy(tmp_path: Path) -> None:
    ...
    with pytest.raises(
        ...
        verify_final_engineering_invocation_authorization_sources(copied_root)


def test_authorization_surfaces_do_not_import_or_invoke_runtime() -> None:
...
def test_frozen_authorization_package_and_verifier() -> None:
    ...
    assert set(registry_entries) == {
        "authorization.json",
        "source-SHA256SUMS",
    }
    assert (
        hashlib.sha256(authorization_path.read_bytes()).hexdigest()
        == registry_entries["authorization.json"]
    )
    assert (
        hashlib.sha256(source_registry.read_bytes()).hexdigest()
        == registry_entries["source-SHA256SUMS"]
    )

    loaded = load_final_engineering_invocation_authorization(
        authorization_path
    )
    assert (
    ...
    assert (
        "FINAL_ENGINEERING_INVOCATION_AUTHORIZATION_VERIFIED=true"
        in completed.stdout
    )
    assert (
...
def test_repository_documentation_records_authorization_authoring() -> None:
    marker = (
        "ADR-104-stage3b-qwake-lc4-e-final-engineering-invocation-"
    ...
    payload = json.loads(
        (PACKAGE_ROOT / "authorization.json").read_text(encoding="utf-8")
    )
    gates = payload["gates"]
    assert gates["final_engineering_invocation_authorization_authored"] is True
    assert gates["final_engineering_invocation_authorization_record_present"] is True
    assert gates["final_engineering_invocation_authorization_issued"] is True
    assert (
        gates["final_engineering_invocation_authorization_post_merge_verified"]
        is False
    )
    ...
    assert gates["operator_phrase_reserved"] is True
