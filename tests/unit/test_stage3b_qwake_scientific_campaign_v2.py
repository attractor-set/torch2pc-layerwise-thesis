...
def test_production_entrypoint_has_no_dynamic_code_or_shell_surface() -> None:
    ...
    source = (root / "scripts/run_stage3b_qwake_scientific_campaign_v2.py").read_text(encoding="utf-8")
...
def _load_scientific_host_launcher():
    ...
    spec = importlib.util.spec_from_file_location(
        "stage3b_qwake_scientific_campaign_host_v2_test_subject",
        path,
    )
...
def test_host_launcher_has_fixed_docker_command_surface() -> None:
    ...
    assert "/workspace/scripts/run_stage3b_qwake_scientific_campaign_v2.py" in string_literals
...
def test_host_output_parent_contract_is_effect_free_and_owner_scoped(
    tmp_path: Path,
) -> None:
    host = _load_scientific_host_launcher()
    ...
    assert (
        host._require_output_parent_preclaim(
            output_root,
            host_uid=os.getuid(),
        )
        == 0o700
    ...
    with pytest.raises(
        ...
        match="parent owner differs from container uid",
    ):
        host._require_output_parent_preclaim(
            output_root,
    ...
    with pytest.raises(
        host.ScientificHostLaunchError,
        match="parent lacks owner write/execute",
    ):
        host._require_output_parent_preclaim(
            output_root,
            host_uid=os.getuid(),
